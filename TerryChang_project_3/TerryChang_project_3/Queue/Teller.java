...
public class Teller
{
                            ...
                            Report toReportTo)
    {
        ...
        SimulationEvent CheckForCustomerEvent = new CheckForCustomerEvent(maxTimeToHelpCustomer, name);
        theEventQueue.add(CheckForCustomerEvent);
    ...
    public class CheckForCustomerEvent extends SimulationEvent
    {
    	...
    	public void process()
    	{
           ...
           if(theLine.isEmpty()){
            serving = null;
            postActionReport = "Customer not found";
           }else{
            serve(theLine.dequeue());
            postActionReport = "Customer served";
           }
            int timeToNext = sharedRandomGenerator.nextInt(maxForHelp);
            SimulationEvent CheckForCustomerEvent = new CheckForCustomerEvent(theEventQueue.getCurrentTime() + timeToNext, myName);
            theEventQueue.add(CheckForCustomerEvent);
